<?php   namespace sunkangchina\phprouter;
use Exception;
/**
...
class Route{
	...
	*/
	...
	function routeClassMatch($class){ 
		  $class = str_replace(static::$r.'\\', '', $class);
		  $a = substr($class,0,strpos($class,'\\'));
		  $b = substr($class,strpos($class,'\\')+1);
		  $cls = static::$r.'\\'.ucfirst($a)."\\".ucfirst($b);
		  if(!class_exists($cls)) {
	...
	*/
	protected function class_exists($class,$ac){
		if(!class_exists($class)) {
	 	...
	 	if(!method_exists($class,$ac."Action")){
	 		header("HTTP/1.1 404 Not Found");
	 		throw new Exception("$ac Action  not exists",404);
	 	}
	} 
